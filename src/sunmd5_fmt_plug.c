...
#else
#ifdef SIMD_COEF_32
#pragma omp parallel for default(none) private(idx) shared(count, ngroups, group_sz, saved_salt, data, input_buf, input_buf_big, out_buf, constant_phrase)
#else
#pragma omp parallel for default(none) private(idx) shared(count, ngroups, group_sz, saved_salt, data, constant_phrase)
#endif // SIMD_COEF_32
