...
#if !defined(JOHN_OCL_WPAPSK_KERNEL) && !defined(_WPAPSK_CUDA_KERNEL) && !defined(__CUDACC__) && !defined(__OPENCL_VERSION__)
#define HOST_CODE
#endif
...
static MAYBE_INLINE void prf_512(uint32_t * key, uint8_t * data, uint32_t * ret)
{
	...
	buff[22] = 0;
	buff[76 + 23] = 0;
	HMAC_Init_ex(ctx, key, 32, EVP_sha1(), NULL);
...
#if defined(HOST_CODE) && !defined(JOHN_OCL_WPAPSK)
